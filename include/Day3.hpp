...
int day3(){
   ifstream infile("/home/tadusz/projekt-2-tadpat12/data/dzien3.txt");;   
  ...
  for(auto v = wektor.begin(); v!= wektor.end(); v++ ){
   std::string s;
    ...
    std::regex word_regex("mul\\((\\d+),(\\d+)\\)|do\\(\\)|don't\\(\\)");
     for (std::smatch sm; std::regex_search(s, sm, word_regex);)
    {
        std::vector<int> w1;
        std::cout << sm.str() << '\n';
        if(sm.str() == "do()"){
        ...
        if(instrukcja){
          w1.push_back(std::stoi(sm[1]));
          w1.push_back(std::stoi(sm[2]));
          w2.push_back(std::move(w1));
        }
        s = sm.suffix();
    }
  }
 ...
 /* auto words_begin = std::sregex_iterator(s.begin(), s.end(), word_regex);
  ...
  if (std::regex_search(s, matches, word_regex)) {
        std::cout << "First number: " << matches[1] << "\n"; // Captured group 1
        std::cout << "Second number: " << matches[2] << "\n"; // Captured group 2
    }
  for (std::sregex_iterator i = words_begin; i != words_end; ++i)
   ...
   std::vector<int> iloczyn;
  //std::cout<< s<<std::endl;
  for (const auto &para : w2) {
      for (auto i : para){
        std::cout<<i<<" ";
      }
     std::cout<<"v"<<std::endl;
  ...
  for (const auto &para : w2) {
      iloczyn.push_back(std::accumulate(para.begin(), para.end(),1,std::multiplies<int>{}));
  }
  licz_mul = std::accumulate(iloczyn.begin(),iloczyn.end(),0);
  std::cout<<licz_mul<<std::endl;
  return 0;
}